...
function sort(sort: Map<string, SortDirection>): (a: any, b: any) => number {
  return (a, b) => {
    for (const [field, direction] of sort.entries()) {
      const val = direction === SortDirection.ASC ? 1 : -1;
...
export default class MockAdapter implements IAdapter {
  ...
  private static generateResult(
    ...
    objects = objects.map(project(query.fields));
  ...
  private validateQuery(query: AdapterQuery) {
    ...
    if (!this._schemas.has(query.object)) {
      console.warn(
        `No objects set for object '${query.object}' – could not validate schema`
      );
      return;
    }
    ...
    if (unmatchedField) {
      throw new Error(
        `Object '${query.object} does not have a field named '${unmatchedField}'`
      );
    }
  }
  ...
  private runSelectCurrentQuery(query: AdapterQuery): Promise<AdapterResult> {
    ...
    const objects = this._objects.get(query.object)!.filter((item) => {
  ...
  private runInsertQuery(query: AdapterQuery): Promise<AdapterResult> {
    if (!this._schemas.has(query.object)) {
      console.warn(
        `Could not validate Schema as there are no Objects for ${query.object}`
      );
    } else {
    ...
    const o = JSON.parse(
      JSON.stringify(Object.fromEntries(query.values.entries()))
    );
