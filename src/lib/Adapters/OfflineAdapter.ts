...
export default class OfflineAdapter implements IAdapter {
  ...
  private createCallback (resolve: any, reject: any, objectName: string) {
    ...
    // create the timeout
    this._timeouts.set(name, window.setTimeout(function () {
  ...
  private async sendCommand (command: string[], objectName: string): Promise<AdapterResult> {
  ...
  private async runSelectCurrentCommand (query: AdapterQuery): Promise<AdapterResult> {
    const objectName = Object.hasOwnProperty.call(CURRENT_CALL_ALIASES, query.object)
      ? CURRENT_CALL_ALIASES[query.object]
      : query.object

    const command = [
      'getDataForObjectV2(' + objectName + ')',
      'fieldName(Id)'
    ]
