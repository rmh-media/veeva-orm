...
it('should select all items', async (t) => {
  const result = await Query.select('Name').from('Account').exec(getAdapter());
...
it('should order the results', async (t) => {
  const result = await Query.select('FirstName', 'LastName')
    .from('Account')
...
it('should add an object', async (t) => {
  ...
  await Query.insert(natascha).into('Account').exec(adapter);
...
it('should delete objects', async (t) => {
  ...
  await Query.deleteFrom('Account').where('FirstName', 'Peter').exec(adapter);

  const count = await Query.select('Name').from('Account').count(adapter);
...
it('should update objects', async (t) => {
  ...
  const results = await Query.select('Name', 'LastName', 'FirstName')
    .from('Account')
    .where('FirstName', 'Peter')
    .exec(adapter);
...
it('should select the current object', async (t) => {
  ...
  const res = await Query.select('Name').fromCurrent('Account').exec(adapter);
...
it('should update the current object', async (t) => {
  ...
  await Query.updateCurrent('Account').set('LastName', 'Padilla').exec(adapter);

  const res = await Query.select('Name', 'LastName')
    .fromCurrent('Account')
    .exec(adapter);
